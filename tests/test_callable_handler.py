...
class TestCallableHandler:
        ...
        def emit_logs():
            for i in range(10):
                record = LogRecord(
                    ...
                    name='test',
                    process=ProcessInfo(id=1, name='test'),
                    thread=ThreadInfo(id=threading.get_ident(), name=threading.current_thread().name),
                    time=datetime(2024, 1, 15)
                )
                handler.emit(record)
    ...
    def test_callable_handler_lambda_function(self, formatter, sample_record):
        ...
        handler = CallableHandler(
            sink=lambda msg: output.append(msg.upper()),
            level=levels.DEBUG,
            formatter=formatter
        )
    ...
    def test_callable_handler_close(self, formatter):
        ...
        handler = CallableHandler(
            sink=lambda msg: output.append(msg),
            level=levels.DEBUG,
            formatter=formatter
        )
