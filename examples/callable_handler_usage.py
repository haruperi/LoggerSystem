...
def example_3_multiple_destinations():
    ...
    log.add(
        sink=error_handler,
        level="ERROR",
        format="[ERROR] {time:YYYY-MM-DD HH:mm:ss} - {message}"
    )
...
def example_4_database_simulation():
    ...
    def log_to_database(json_str):
        ...
        print(f"[DB] Inserted: ID={db_record['id']}, Level={db_record['level']}, Message={db_record['message'][:40]}...")
    
    # Create logger
    log = Logger()
    log.add(
        sink=log_to_database,
        level="INFO",
        serialize=True
    )
...
def example_6_lambda_and_class_methods():
    ...
    log1 = Logger()
    log1.add(
        sink=lambda msg: print(f"[Lambda] {msg}"),
        level="INFO",
    ...
    class LogAnalyzer:
        ...
        def analyze_log(self, json_str):
            ...
            print(f"[Analyzed] {data['message'][:40]}... (Total: {self.log_count}, Errors: {self.error_count})")
...
def example_7_filtering_and_processing():
    ...
    log.add(
        ...
        level="INFO",
        serialize=True,
        filter=lambda record: record.extra.get('priority') == 'high'  # Filter for high priority only
    )
...
def example_8_real_world_use_case():
    ...
    log.add(
        ...
        level="INFO",
        serialize=True,
        filter=lambda r: 'action' in r.extra
    )
