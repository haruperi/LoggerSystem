...
def example_1_global_extra():
    ...
    logger = Logger()
    logger.add(sys.stderr, level="INFO", format="{time:HH:mm:ss} | {level: <8} | {message} | app_version={extra.app_version}")
...
def example_2_bind_basic():
    ...
    logger = Logger()
    logger.add(sys.stderr, level="INFO", format="{time:HH:mm:ss} | {message} | request_id={extra.request_id}")
...
def example_5_nested_contextualize():
    ...
    logger.add(
        sys.stderr,
        level="INFO",
        format="{message} | req={extra.request_id} | task={extra.task_id}"
    )
...
def example_7_database_transaction():
    ...
    logger.add(
        sys.stderr,
        level="DEBUG",
        format="{level: <8} | [{extra.transaction_id}] {message}"
    )
