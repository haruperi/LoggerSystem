...
def example_4_module_filter_include():
    ...
    logger.add(
        sys.stdout, level="INFO", filter=module_filter, format="{level} | {module} | {message}"
    )
...
def example_7_multiple_handlers_different_filters():
    ...
    logger.add(
        sys.stdout, level="TRACE", filter=info_filter, format="[CONSOLE] {level} | {message}"
    )
    ...
    logger.add(
        error_stream, level="TRACE", filter=error_filter, format="[ERROR FILE] {level} | {message}"
    )
    ...
    logger.add(
        audit_stream, level="TRACE", filter=audit_filter, format="[AUDIT FILE] {level} | {message}"
    )
...
def example_8_custom_filter_class():
    ...
    security_filter = KeywordFilter(
        keywords=["security", "authentication", "authorization", "breach"], case_sensitive=False
    )
...
def example_12_time_based_filter():
    ...
    logger.add(
        sys.stdout, level="INFO", filter=business_hours, format="{time:HH:mm} | {level} | {message}"
    )
...
def example_13_complex_filter_combination():
    ...
    high_severity = OrFilter(
        lambda r: r.level.name == "ERROR", lambda r: r.level.name == "CRITICAL"
    )

    database_related = OrFilter(
        lambda r: "database" in r.message.lower(), lambda r: "connection" in r.message.lower()
    )
