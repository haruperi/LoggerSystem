...
def example_6_with_extra_context():
    ...
    handler.emit(
        create_sample_record(
            levels.INFO, "User logged in", extra={"user_id": "user123", "request_id": "req-456"}
        )
    )

    handler.emit(
        create_sample_record(
            levels.INFO, "Data processed", extra={"user_id": "user456", "request_id": "req-789"}
        )
    )
...
def example_7_custom_filter():
    ...
    handler = FileHandler(
        "logs/important.log", levels.DEBUG, formatter, filter_func=important_filter
    )
...
def example_11_production_setup():
    ...
    app_formatter = Formatter("{time:YYYY-MM-DD HH:mm:ss} | {level.name: <8} | {message}")
    ...
    print(
        f"  app.log: {len(Path('logs/production/app.log').read_text().strip().split(chr(10)))} lines"
    )
    print(
        f"  debug.log: {len(Path('logs/production/debug.log').read_text().strip().split(chr(10)))} lines"
    )
    print(
        f"  error.log: {len(Path('logs/production/error.log').read_text().strip().split(chr(10)))} lines"
    )
