...
def example_4_colorization():
    ...
    handler_color = StreamHandler(sys.stdout, levels.INFO, formatter, colorize=True)

    # Explicitly disable colorization
    handler_no_color = StreamHandler(sys.stdout, levels.INFO, formatter, colorize=False)
...
def example_5_custom_filter():
    ...
    formatter = Formatter("{level.name} | {message}")
    handler = StreamHandler(sys.stdout, levels.DEBUG, formatter, filter_func=important_filter)
