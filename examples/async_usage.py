...
def example_8_graceful_shutdown():
    ...
    # Add async handler
    handler_id = logger.add(sys.stdout, level="INFO", format="{message}", enqueue=True)
...
def example_11_async_error_handling():
    ...
    logger.add(sometimes_failing_sink, level="INFO", format="{message}", enqueue=True)
...
def example_12_practical_web_app():
    ...
    logger.add(
        "logs/webapp_errors.log", level="ERROR", format="{time} | {level} | {message}", enqueue=True
    )
