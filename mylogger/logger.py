...
from .handler import StreamHandler, FileHandler, CallableHandler
...
class Logger:
    ...
    def contextualize(
        self, **kwargs
    ) -> "ContextManager":  # pyright: ignore[reportUndefinedVariable]
    ...
    def _log(self, level: Union[str, int], message: str, *args, _depth: int = 2, **kwargs) -> None:
        ...
        if exception_info is True:
            ...
            exception_info = ExceptionInfo(
                type=exception_info[0], value=exception_info[1], traceback=exception_info[2]
            )
        ...
        try:
            process_info = ProcessInfo(id=os.getpid(), name=multiprocessing.current_process().name)
        except Exception:
            process_info = ProcessInfo(id=os.getpid(), name="MainProcess")

        thread_info = ThreadInfo(id=threading.get_ident(), name=threading.current_thread().name)
...
try:
    ...
    logger.add(
        "logs/errors.log", level="ERROR", rotation="100 MB", compression="gz", retention="90 days"
    )
except Exception:
