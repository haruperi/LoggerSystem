...
from typing import Any, Callable, Optional, TextIO, Union, TYPE_CHECKING
...
if TYPE_CHECKING:
    from .record import LogRecord, Level
...
class StreamHandler(Handler):
    ...
    def __init__(self, sink: TextIO, level: Level, formatter: Formatter, **options):
    ...
    def _open_file(self) -> None:
        ...
        try:
            self.file_handle = open(
                self.path, mode=self.mode, encoding=self.encoding, buffering=self.buffering
            )
        except Exception as e:
    ...
    def _rotate(self) -> None:
        ...
        try:
            ...
            if self.path.exists():
                ...
                if self.compression:
                    try:
                        # Compress the rotated file (may delete original if keep_original=False)
...
class CallableHandler(Handler):
    ...
    def __init__(self, sink: Callable, level: Level, formatter: Formatter, **options):
